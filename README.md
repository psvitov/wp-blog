...
Для установки в других облачных сервисах необходимо изменить описание провайдера в файле main.tf
...
### Ответ:
traceroute - программа построения маршрута от источника до определенного адресата, выстраивает цепочку сетевых устройств путем пересылки запроса от устройства к устройству
...
### Ответ:
SWAP - буфер виртуальной памяти, используемый как файл подкачки при нехватке аппаратной ОЗУ, так же используется для гибернации - сохраняет образ памяти во время сна. 


# Локальная установка:

Можно установить блог и мониторинг локально, без использования облачных сервисов:

1. Для установки блога на компьютере/виртуальной машине должны быть установлены: Docker, Docker-compose, Git
2. Так же скачиваем репозиторий по ссылке

```
git clone https://github.com/psvitov/wp-blog.git
```

3. Переходим в каталог `wp`
4. Так же как и в варианте с использованием облачной инфраструктуры меняем данные в файлах `.env` и `site.env`на необходимые
5. Последовательно запускаем команды:

```
sudo docker compose -f docker-compose.yml up -d

sudo docker compose exec -T wp-cli sh < settings.sh
```
Wordpress-блог запущен

6. Для установки мониторинга необходимо вернуться в корневой каталог репозитория и перейти в каталог `monitoring`
7. Запускаем команду

```
sudo docker compose -f docker-compose.yml up -d
```
Мониторинг запущен, осталось настроить Grafana по аналогии с настройкой в облачной среде
